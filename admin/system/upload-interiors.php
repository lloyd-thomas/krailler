...
if (isset($_POST['ajax']) && $_POST['ajax'] == 1) {
  ...
  if (isset($_FILES['collection']['type']) && $_FILES['collection']['type'] == 'image/jpeg') {
    ...
    move_uploaded_file($_FILES['collection']['tmp_name'], $peel."/../../img/interiors/".$image['cid'].'-'.$image['id'].'-'.$image['slug'].".jpg");
    $resizeObj = new resize($peel."/../../img/interiors/".$image['cid'].'-'.$image['id'].'-'.$image['slug'].".jpg");
    $resizeObj -> resizeImage(150, 150, 'crop');
    $resizeObj -> saveImage($peel."/../../img/interiors/thumbs/".$image['cid'].'-'.$image['id'].'-'.$image['slug']."-1.jpg", 100);
    $resizeObj -> resizeImage(1200, 1200, 'auto');
    $resizeObj -> saveImage($peel."/../../img/interiors/thumbs/".$image['cid'].'-'.$image['id'].'-'.$image['slug']."-2.jpg", 100);
...
<tr>
  <td><img src="/img/interiors/thumbs/<?php echo $pimages[$x]['cid']; ?>-<?php echo $pimages[$x]['id']; ?>-<?php echo $pimages[$x]['slug']; ?>-1.jpg?<?php echo rand(1000,9999); ?>" style="width: 50px;" /></td>
  <td><?php echo $pimages[$x]['title']; ?></td>
  <td>
 <?php if ($x<>0) { ?>
          <a href="?edit=<?php echo $_POST['editid']; ?>&amp;pup=<?php echo $x; ?>#interiors" class="btn btn-primary"><i class="fa fa-hand-o-up"></i> Up</a>
        <?php } else { ?>
          <a href="" class="btn btn-default" disabled='disabled'><i class="fa fa-hand-o-up"></i> Up</a>
        <?php } ?>
        <?php if ($x<>sizeof($pimages)-1) { ?>
        <a href="?edit=<?php echo $_POST['editid']; ?>&amp;pdown=<?php echo $x; ?>#interiors" class="btn btn-primary"><i class="fa fa-hand-o-down"></i> Down</a>
        <?php } else { ?>
        <a href="" class="btn btn-default" disabled='disabled'><i class="fa fa-hand-o-down"></i> Down</a>
        <?php } ?>

  </td>
  <td align="right"><a href="?edit=<?php echo $_POST['editid']; ?>&amp;pdelete=<?php echo $pimages[$x]['id']; ?>#interiors" class="btn btn-danger" onclick="return confirmClick('Are you sure you want to delete this?');" ><i class="fa fa-trash-o"></i> Delete</a></td>
</tr>
<?php } ?>
</tbody>
</table>
